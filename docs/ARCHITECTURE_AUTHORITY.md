...
| Local Meshy-native character ZIP intake and direct same-character Walking inspection | `src/vandrel_foundry/services/add_meshy_native_character_package.py` retains the exact four-root source union; `src/vandrel_foundry/services/inspect_meshy_native_character.py` produces bounded neutral playback and 24-joint compatibility evidence; `src/vandrel_foundry/blender/inspect_meshy_native_character.py` owns FBX inspection/rendering. This is Foundry technical evidence only, not provider provenance recovery, a shared-animation claim, Vandrel adoption, or gameplay authority. |
