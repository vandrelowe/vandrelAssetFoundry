...
| Meshy-native multi-entry motion intake | `src/vandrel_foundry/services/add_meshy_native_multi_motion_package.py` owns the exact one-ZIP, twenty-withSkin-FBX, one-texture immutable source union and transactional intake; `src/vandrel_foundry/domain/meshy_native_multi_motion.py` owns its closed provenance report. It retains every entry root, excludes only the declared legacy container-axis outlier from assembly, and does not assign runtime or gameplay authority. |
| Meshy-native real-character motion assembly and continuous evidence | `src/vandrel_foundry/services/assemble_meshy_native_character_motion.py` owns exact six-root or extended 28-root lineage, transactional processing, collision-safe runtime action identity, independent exported-GLB skin proof, semantic evidence, proportional continuous playback, and readiness gaps; `src/vandrel_foundry/domain/meshy_native_character_motion.py` owns the closed evidence shape; `src/vandrel_foundry/blender/assemble_meshy_native_character.py` maps native-joint global posed orientations through one constant armature correction and reconstructs target-local bases through posed parents while retaining target rest translations, bone lengths, mesh, bind matrices, material, and texture and deterministically normalizing skin weights to the bounded top-four export policy. |
| Meshy-native assembled-character technical release evidence | `src/vandrel_foundry/services/validate_meshy_native_character_release.py` owns the bounded exact-model Godot playback and independent skin/material/texture/H4 verification transaction; `src/vandrel_foundry/domain/meshy_native_release.py` owns its closed report. This route packages 29 exact clip names and evidence without gameplay mappings and remains explicitly unaccepted by Vandrel runtime authority. |
