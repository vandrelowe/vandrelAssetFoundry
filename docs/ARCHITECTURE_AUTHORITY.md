...
| Meshy-native real-character motion assembly and continuous evidence | `src/vandrel_foundry/services/assemble_meshy_native_character_motion.py` owns exact six-root lineage, transactional processing, independent exported-GLB skin proof, semantic evidence, playback, and readiness gaps; `src/vandrel_foundry/domain/meshy_native_character_motion.py` owns the closed evidence shape; `src/vandrel_foundry/blender/assemble_meshy_native_character.py` applies native-joint rest-space motion while retaining the target mesh, bind matrices, material, and texture and deterministically normalizing skin weights to the bounded top-four export policy. |
