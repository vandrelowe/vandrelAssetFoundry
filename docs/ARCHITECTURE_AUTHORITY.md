...
| Meshy-native real-character motion assembly and continuous evidence | `src/vandrel_foundry/services/assemble_meshy_native_character_motion.py` owns the complete character, canary, and every numbered multi-motion root union, transactional processing, curve-level collision-safe runtime action identity, independent exported-GLB skin proof, semantic evidence, proportional continuous playback, and readiness gaps; `src/vandrel_foundry/domain/meshy_native_character_motion.py` owns the closed evidence shape; `src/vandrel_foundry/blender/assemble_meshy_native_character.py` maps native-joint global posed orientations through one constant armature correction and reconstructs target-local bases through posed parents while retaining target rest translations, bone lengths, mesh, bind matrices, material, and texture and deterministically normalizing skin weights to the bounded top-four export policy. |
