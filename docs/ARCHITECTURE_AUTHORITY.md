...
| Meshy-native merged-animation intake, fixed 24-joint normalization/splitting, and canary playback evidence | `src/vandrel_foundry/services/add_meshy_native_animation_package.py`, `src/vandrel_foundry/services/normalize_meshy_native_animations.py`, and `src/vandrel_foundry/services/render_meshy_native_playback.py`; report schema in `src/vandrel_foundry/domain/meshy_native_canary.py`; bounded adapters in `src/vandrel_foundry/blender/normalize_meshy_native_animations.py` and `src/vandrel_foundry/blender/render_meshy_native_playback.py` |
| Local Meshy-native character intake and direct same-character locomotion inspection | `src/vandrel_foundry/services/add_meshy_native_character_package.py` retains either the historical exact ZIP/character/Walking/texture root union or the exact API character-FBX/character-GLB/Walking-FBX/Running-FBX root union; API texture bytes are explicitly derived from the exact provider GLB rather than laundered as an independent root. `src/vandrel_foundry/services/inspect_meshy_native_character.py` produces bounded neutral playback and 24-joint compatibility evidence, including Running structure for API intake; `src/vandrel_foundry/blender/inspect_meshy_native_character.py` owns FBX inspection/rendering. This is Foundry technical evidence only, not broader provider rights, a shared-animation claim, Vandrel adoption, or gameplay authority. |
| Meshy-native multi-entry motion intake | `src/vandrel_foundry/services/add_meshy_native_multi_motion_package.py` owns each numbered exact one-ZIP, all-withSkin-FBX, one-texture immutable source union and transactional intake; `src/vandrel_foundry/domain/meshy_native_multi_motion.py` owns its closed provenance report. Historical package 001 retains its exact twenty-FBX policy. Later packages retain every safe entry root and may exclude only a separately proven incompatible container outlier; filename and carrier labels remain provenance rather than compatibility or gameplay authority. |
| Meshy-native real-character motion assembly and continuous evidence | `src/vandrel_foundry/services/assemble_meshy_native_character_motion.py` owns the complete character, canary, and every numbered multi-motion root union, transactional processing, curve-level collision-safe runtime action identity, independent exported-GLB skin/material proof, semantic evidence, proportional continuous playback, and readiness gaps; `src/vandrel_foundry/domain/meshy_native_character_motion.py` owns the closed evidence shape; `src/vandrel_foundry/blender/assemble_meshy_native_character.py` maps native-joint global posed orientations through one constant armature correction and reconstructs target-local bases through posed parents while retaining target rest translations, bone lengths, mesh, and bind matrices. Historical schemas retain the bounded top-four export policy. The candidate-only schema-1.5 repair profile owns deterministic top-eight export, opaque base-color-only material normalization, and synchronized provider-FBX/r001/repaired neutral-gray pose/skin-continuity evidence; that evidence does not visually accept material appearance, assert Vandrel runtime acceptance, or rewrite a prior release. |
| Meshy-native assembled-character technical release evidence | `src/vandrel_foundry/services/validate_meshy_native_character_release.py` owns the bounded exact-model Godot playback and independent skin/material/texture/H4 verification transaction; `src/vandrel_foundry/domain/meshy_native_release.py` owns its closed report. This route packages the exact current clip inventory, complete current source-root union, and registered playback evidence without gameplay mappings and remains explicitly unaccepted by Vandrel runtime authority. |
...
| Explicit immutable single or bounded-set library publication | `src/vandrel_foundry/services/publish_release.py` |
