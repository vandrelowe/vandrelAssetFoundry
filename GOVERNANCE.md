...
- `C:\dev\Vandrel`: read-only by default. The downstream-integration exception
  below permits narrowly scoped consumer validation writes.

Tests use temporary directories and must never touch real workspaces or sibling
repositories.

### Standing downstream-integration exception

For an approved, release-enabled asset already published as an immutable
asset-library revision, an AI development session may perform the consumer-side
integration step in the configured Vandrel checkout without another per-file
approval. This standing authority is limited to:

- copying that exact release into a new asset-scoped runtime content path;
- adding or updating its asset-scoped wrapper and bounded debug/test scene;
- allowing Godot to import those files and running a finite bomb test;
- writing asset-scoped validation evidence generated by that test.

The session must inspect and preserve existing unrelated changes, use only
Vandrel-owned runtime paths allowed by current Vandrel contracts, and report
the exact files written. This exception does not authorize overwriting another
asset, editing `Main.tscn`, runtime catalogs, gameplay registries, ECS or
gameplay behavior, deleting files, committing, pushing, or claiming canonical
runtime acceptance. Those actions still require separate explicit intent. It
also does not let Foundry package metadata assign authoritative `res://` paths;
the wrapper and test remain Vandrel-owned consumer work.
...
- Cross-repository compatibility: fixture-based contract tests by default.
  The standing downstream-integration exception may use the configured Vandrel
  checkout for an approved release's bounded consumer test.
